...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        function load_gallery(){
         ...
         $('#file').change(function(){// change có nghĩa là thay đổi (thay đổi là khi ta chọn ảnh)
             var error= '';
             var files =$('#file')[0].files; // (khi up file ảnh - tức là file đầu tiên được gán vào vị trí 0)
             if(files.length>5){
                error+='<p>Bạn chỉ được chọn tối đa 5 ảnh</p>'
             }else if(files.length==''){
                error+='<p>Bạn không được bỏ trống trường này</p>'
             }else if(files.size >10000000){ // nếu file lớn hơn 2mb
                 error+='<p>File ảnh không được lớn hơn 10MB</p>'
             }
              
             if(error==''){ // nếu k có lỗi thì để trống
         ...
         $(document).on('blur','.edit_gal_name',function(){
             var gal_id = $(this).data('gal_id'); // lấy ra id của đoạn văn bản đó và sau đó lấy ra đoạn text của văn bản:
             ...
             $.ajax({
                ...
                success:function(data){
                    load_gallery(); // gọi hàm để load lại thư viện ảnh và xuất ra thông báo cập nhật thành công:
                    $('#error_gallery').html('<span class="text-danger"> Cập nhật tên hình ảnh thành công</span>');
                }
            });
         });
          $(document).on('click','.delete-gallery',function(){
             var gal_id = $(this).data('gal_id'); // lấy ra id 
           ...
           $(document).on('change','.file_image',function(){
             var gal_id = $(this).data('gal_id'); // lấy ra id 
             var image =document.getElementById("file-"+gal_id).files[0]; // lấy thành phần bởi id
